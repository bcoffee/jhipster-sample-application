...
export class InstallationService {
    ...
    create(installation: IInstallation): Observable<EntityResponseType> {
        return this.http.post<IInstallation>(this.resourceUrl, installation, { observe: 'response' });
    }

    update(installation: IInstallation): Observable<EntityResponseType> {
        return this.http.put<IInstallation>(this.resourceUrl, installation, { observe: 'response' });
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http.get<IInstallation>(`${this.resourceUrl}/${id}`, { observe: 'response' });
    }

    query(req?: any): Observable<EntityArrayResponseType> {
        const options = createRequestOption(req);
        return this.http.get<IInstallation[]>(this.resourceUrl, { params: options, observe: 'response' });
    }
